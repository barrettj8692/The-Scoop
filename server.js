...
function deleteComment(url, request){
  ...
  console.log(request);
  if(savedComment) {
    const articleRef = savedComment.articleId;
    const articleComment = database.articles[articleRef].commentIds;
    articleComment.splice(articleComment.indexOf(id), 1);
...
function loadDatabase(){
  try {
    const doc = yaml.safeLoad(fs.readFileSync('./data.yml', 'utf8'));
    console.log(doc);
    return doc;
  } catch (e) {
    console.log(e);
